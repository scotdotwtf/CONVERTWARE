...
----------------------------------------------------------------
print("> Running Dual Ultima by Sugarie Saffron")
...
function attackone()
        attack=true
		tr1.Enabled = true
        local con=Hitbox.Touched:connect(function(hit) Damagefunc() end) 
        for i=0,1,0.1 do
                swait()
...
function attack2one()
        attack=true
		tr2.Enabled = true
        local con=Hitbox2.Touched:connect(function(hit) Damagefunc() end) 
        for i=0,1,0.1 do
                swait()
RootJoint.C0=clerp(RootJoint.C0,RootCF*cf(0,0,0)*angles(math.rad(0),math.rad(0),math.rad(70)),.3)
...
local con2=Hitbox2.Touched:connect(function(hit) Damagefunc() end) 
...
local hhat2 = alignchar['ShadowBladeMasterAccessory']
...
AlignCharacter(hhat.Handle,Character.Model.Handle,Vector3.new(-0.25,2.5,0),Vector3.new(0,0,-125))
AlignCharacter(hhat2.Handle,Character.model2.Handle,Vector3.new(-0.25,2.5,0),Vector3.new(0,0,-125))

for _,v in pairs(Character.Model:GetChildren()) do
	if v:IsA("BasePart") then
		v.Transparency = 1
	end
end

for _,v in pairs(Character.model2:GetChildren()) do
	if v:IsA("BasePart") then
		v.Transparency = 1
	end
end
